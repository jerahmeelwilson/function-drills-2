...
// CODE HERE
const totalPopulation = populations.reduce((sum,element) => sum + element);
...
// CODE HERE
const totals = orders.map(element => element.price * (1+element.tax));
...
*/
// CODE HERE
const bobsTotal = purchases.filter(element => element.owner === "Bob").map(element => element.price).reduce((sum, element)=> sum + element);
